module protocol_sui::continuous_indexing {
    ...
    public fun update_index(
        indexing: &mut ContinuousIndexing,
        new_rate: u32,
        ...
        let current_index = calculate_current_index(
            indexing.latest_index,
            indexing.latest_rate,
            indexing.latest_update_timestamp,
            current_timestamp
    ...
    public fun calculate_current_index(
        ...
        // Safe to use unchecked since current_timestamp >= latest_update_timestamp
        ...
        let rate_scaled = continuous_indexing_math::convert_from_basis_points(latest_rate);
        let delta_index = continuous_indexing_math::get_continuous_index(
            rate_scaled,
            (time_elapsed as u32)
        );
        
        // Multiply indices and cap at u128 max
        let result = continuous_indexing_math::multiply_indices_down(latest_index, delta_index);
